#!/usr/bin/env bash
# scripts/run_dbscan_comparison.sh
#
# Runs the full DBSCAN vs GRACE GroupDetector comparison table (Table 1).
# Outputs an ASCII table + LaTeX snippet for the CoRL paper.
# Results are saved to:  results/perception_detection_results.txt
#
# Prerequisites:
...
#
# Usage:
#   bash scripts/run_dbscan_comparison.sh              # uses saved results.pt (fast)
#   bash scripts/run_dbscan_comparison.sh --force      # re-runs model inference from scratch
...
echo "Done. Results saved to: $RECORD"
