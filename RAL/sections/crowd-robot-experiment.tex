...
\begin{figure*}[t]
    \centering
    \resizebox{\textwidth}{!}{\includegraphics{figures/com.pdf}}
    \vspace{-25pt}
...
\subsection{Realistic Crowd Simulation Benchmark}
We introduce a benchmark environment extending the framework of~\cite{liu2023intention} with five empirically motivated modeling phases that substantially increase realism over prior simulation settings.

\textbf{Phase A — Individual speed variation.}
Each pedestrian's preferred speed is drawn from $\mathcal{N}(1.34,\,0.26)$~m/s, clipped to $[0.8,\,1.8]$~m/s, following Weidmann (1992). This produces heterogeneous crowd flow rather than the uniform-speed assumption common in prior work.

\textbf{Phase B — Group speed coupling.}
Dynamic groups travel at $0.85 \times \min_{i \in G} v_{\mathrm{pref},i}$~m/s~\cite{moussaid2010walking}, reflecting the empirical finding that groups naturally slow to accommodate their slowest member.

\textbf{Phase C — F-formation static groups.}
Stationary groups are initialized in one of four sociologically documented F-formations (vis-à-vis, L-shape, side-by-side, circle)~\cite{kendon1990conducting}, ensuring spatial realism for conversational clusters.

\textbf{Phase D — Leader--follower dynamics.}
Moving groups have a randomly selected leader navigating via ORCA; followers maintain staggered lateral slots with inter-follower repulsion inspired by Helbing \& Molnár (1995):
\begin{equation}
v_i = v_{\mathrm{leader}} + k(c - p_i) + f_{\mathrm{rep},i},
\end{equation}
where $k$ is a cohesion factor, $c$ is the group centroid, and $f_{\mathrm{rep},i}$ prevents follower--follower collisions.

\textbf{Phase E — Convex-hull boundaries.}
Group spatial boundaries are computed as convex hulls of member positions at each timestep, replacing fixed-radius circles. These dynamic polygon boundaries are used for both GCR computation and reward shaping during training of learned baselines.

The arena is a circle of radius 8.5~m with 20 humans organized into 3 groups (3--4 members each, two placed along the robot's path) and remaining individuals navigating independently. Three group types appear: \textit{static\_f} (stationary F-formations), \textit{dynamic\_lf} (leader--follower), and \textit{dynamic\_free} (each member navigates independently via ORCA). The robot is holonomic with a 5~m sensing range. Groups are detected in real-time via DBSCAN ($\epsilon = 1.5$~m, minimum cluster size = 2), without access to ground-truth group labels.
...
\subsection{Implementation Details}
We evaluate five navigation baselines: ORCA~\cite{orca}, Social Force (SF)~\cite{helbing1995social}, DS-RNN~\cite{liu2020decentralized}, AG-RL~\cite{liu2023intention}, and GARN~\cite{garn2025}. TAGA is applied on top of each baseline except GARN, which explicitly trains with group-awareness and is evaluated standalone. Key TAGA parameters: safety margin $S = 0.5$~m, critical distance $d_{\mathrm{critical}} = 0.3$~m, personal distance $d_{\mathrm{personal}} = 0.5$~m. All methods are evaluated on 500 randomly generated episodes with fixed seeds for reproducibility. The TAGA safety controller operates at 10~Hz.
