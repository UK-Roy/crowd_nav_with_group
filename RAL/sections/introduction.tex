...
\begin{enumerate}
    \item \textbf{Real-time group detection.} D
    We develop a DBSCAN-based module that enables real-time identification of human groups from observable motion cues such as position, velocity, and proximity. 
    Unlike prior methods that assume pre-labeled group membership, our approach dynamically detects, updates, and dissolves group formations as they evolve, addressing a critical gap in socially compliant robot navigation.

    \item \textbf{Group-aware tangent navigation (TAGA).} \UT{Try to address the 2nd review of reviwer 31281}
    We propose TAGA, a modular group-avoidance mechanism that computes tangent trajectories around detected groups while ensuring individual safety. 
    TAGA integrates seamlessly with existing navigation frameworks (both reactive and learning-based) without retraining, and consistently improves social compliance while preserving navigation efficiency.

    \item \textbf{Hierarchical safety control.} 
    We design a hierarchical safety controller that maintains graded safety zones around both individuals and groups, enabling robots to execute tangent maneuvers safely even in dense, mixed populations. 
    This controller provides robust, low-level safety during cooperative group-aware navigation.

    \item \textbf{Enhanced simulation and benchmarking framework.} 
    We extend the crowd simulation environment to model realistic group dynamics, including clustering behaviors, leader–follower patterns, and dynamic group formations. 
    This framework supports reproducible benchmarking across varying densities, group ratios, and motion patterns.

    \item \textbf{Continuous evaluation metric (GCR).} \UT{Try to address the 4th review of reviwer 31281 and also mention in section IV-C}
    We introduce the \textit{Group Collision Rate (GCR)} as a complementary metric to conventional terminal-state measures such as Success Rate (SR), Collision Rate (CR), and Timeout Rate (TR). 
    GCR quantifies the percentage of navigation time the robot spends within group boundaries, providing continuous, frame-level assessment of social compliance throughout navigation episodes.

    \item \textbf{Comprehensive empirical validation.} 
    We conduct extensive benchmarking across diverse crowd scenarios—dense groups, mixed populations (50\% individuals, 50\% in groups), and dynamic formations. 
    Integrating TAGA with established navigation methods including ORCA~\cite{orca}, Social Force~\cite{helbing1995social}, DS-RNN~\cite{liu2020decentralized}, and AG-RL~\cite{liu2023intention}, we demonstrate substantial reductions in group intrusions while maintaining or improving navigation success rates. \SR{I checked upto this point}
\end{enumerate}
