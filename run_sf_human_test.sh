#!/bin/bash
# Run test.py for all policies in individuals-only environment with Social Force pedestrians.
# No groups, realistic phases A-E on, human policy = social_force.
# All runs use seed=425 → same 100 episodes for every model.
# Logs saved to results/test_sf_human/

set -e
OUTDIR="results/test_sf_human"
mkdir -p "$OUTDIR"
...
# Quick SR summary
echo ""
echo "=== SR Summary (individuals-only, social force pedestrians) ==="
